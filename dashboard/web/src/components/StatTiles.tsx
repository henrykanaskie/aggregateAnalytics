...
export default function StatTiles({ rows, allRows, statKey, stat, line }: { rows: GameRow[]; allRows: GameRow[]; statKey: string; stat?: StatDef; line: number | null }) {
  ...
  const Rate = ({ k, x, sub }: { k: string; x: typeof s; sub?: string }) => (
    <div className="tile">
      ...
      <div className="s">{line === null ? `set a ${w.line}` : `${x.over}-${x.under}${x.push ? `-${x.push}` : ""} ${w.over} · avg ${f(x.avg)}`}{sub ? ` · ${sub}` : ""}</div>
  ...
  return (
    <div className="tiles">
      ...
      <div className="tile"><div className="k">Streak</div><div className="v" style={{ color: s.streak ? (s.streak.side === "over" ? "var(--over)" : "var(--under)") : undefined }}>{s.streak ? `${s.streak.n} ${s.streak.side === "over" ? w.over : w.under}` : "–"}</div><div className="s">consecutive vs {w.line}</div></div>
      <div className="tile"><div className="k">{w.line} vs avg</div><div className="v">{line !== null && s.avg !== null ? (s.avg - line > 0 ? "+" : "") + (s.avg - line).toFixed(1) : "–"}</div><div className="s">avg minus {w.line}</div></div>
    </div>
  );
}
