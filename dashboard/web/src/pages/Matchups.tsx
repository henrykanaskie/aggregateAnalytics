...
// A listing next to a name: red for a designation that means out, amber for
// anything short of that (questionable, sat out or limited in practice).
const Listed = ({ status, injury }: { status?: string | null; injury?: string | null }) =>
  status ? <span className={`pill ${status === "Out" || status === "Doubtful" ? "under" : "warn"}`} title={injury ?? undefined}>{status}</span> : null;
  ...
  return (
    <div className="grid" style={{ gap: 14 }}>
      ...
      <div className="grid" style={{ gap: 14 }} data-tour="matchup-sides">
        {d.sides.map((s) => <SideView key={s.offense} s={s} metrics={d.metrics} labels={d.dvp_labels} usageSeason={d.season_used} />)}
      </div>
...
function SideView({ s, metrics, labels, usageSeason }: { s: MatchupSideFull; metrics: TeamMetric[]; labels: Record<string, string>; usageSeason: number }) {
