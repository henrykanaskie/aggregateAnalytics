    ...
    <div className="grid grid-main">
      <div className="panel" data-tour="results-angles">
        ...
        <div className="tbl-wrap"><table className="tbl">
          <thead><tr><th className="left">Angle</th>{kind === "all" && <th className="left res-kind">Kind</th>}<th>Record</th><th>Right</th><th title="Against the closing prop lines, one call per player line: a player angle's own line, or each of a team angle's players' lines">vs lines</th><th></th></tr></thead>
        ...
        <details style={{ marginTop: 8 }}>
          ...
          <div className="hint" style={{ marginTop: 6 }}>An angle is right when the number it was about landed on the side it said: the team's rate against its own pre-game number, the player's game against his previous 16, and a "generous to RBs" kind of angle on what the offense's RBs usually get, not the league average. Separately, "vs line" asks the betting question: did the players' closing prop lines land on the side the angle leaned? A player angle is his own line; a team angle is every line of the players it points at (its backs' rushing yards for a box angle, the receivers' yards for a pass defense), each player one call. This season only (last season until a game of this one is graded). Rebuilt from what was known before kickoff. Every call is a hit or a miss, with no margin: a move the way the angle said is a hit, anything else, a tie included, is a miss. Against a line, landing exactly on it is a miss too. A call on a player who got hurt in the game (under half his usual snaps, then on the next week's injury report) still counts, and is marked "got hurt" so you can read it knowing that. Box angles only count games where that box actually showed up on at least one of the carries they were about (FTN charting); the rest are left out too. Colour needs five or more.</div>
        </details>
      </div>
