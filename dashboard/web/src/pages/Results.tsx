    ...
    <div className="grid grid-main">
      <div className="panel" data-tour="results-angles">
        ...
        <div className="small muted" style={{ marginBottom: 8 }}>
          {span} · {data.by_kind.map((k) => <span key={k.kind}>{k.kind} {k.hits}/{k.n} ({fmtPct(k.hits / k.n)}) </span>)}{data.line_n ? `· vs the line ${data.line_hits}/${data.line_n} (${fmtPct((data.line_hits ?? 0) / data.line_n)}) ` : ""}{data.absent ? ` · ${data.absent} box calls where the box never showed up` : ""}{data.injured ? ` · ${data.injured} calls on players who got hurt, not counted` : ""}
        </div>
